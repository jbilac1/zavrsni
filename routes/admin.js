...
router.delete('/kolegiji/:id', isLoggedIn, catchAsync(async (req, res) => {
    if(req.user.username === "admin"){
        const { id } = req.params;
        const kolegij = await Kolegij.findById(id);
        if(kolegij.studenti){
            kolegij.studenti.forEach(async (student)=>{
                await Student.findByIdAndUpdate(student._id, { $pull: { kolegij: id } });
            })
        }
        if(kolegij.ispiti){
...
router.delete('/studenti/:id', isLoggedIn, catchAsync(async (req, res) => {
    if(req.user.username === "admin"){
        ...
        if(student.kolegiji){
            student.kolegiji.forEach(async (kolegij)=>{
                await Kolegij.findByIdAndUpdate(kolegij._id, { $pull: { studenti: id } });
            })
        }
